...
class Data_Processor:
    ...
    def data_reader (self):
        location = os.path.join(os.path.expanduser("~"), 'data', 'datasets')
        fil = location + '/' + str(self.dataset) + '_dataset' + '.csv'
        data = pd.read_csv(fil, sep = ',', encoding = 'utf-8')
        # if data.columns == ['Unnamed:0']: 
        data = data.drop('Unnamed: 0', axis = 1) 
        return data

    def canonical (self, dataframe): # if less than x% dropped, keep, else find ways to address it
        a, b = [], []
        for i in dataframe['smiles']: # Check if all the molecules are unique, pyTDC 
...
# tokenizer = RobertaTokenizer('/home/suryabalaji/GPT_MolBERTa/roberta/tokenizer_folder/vocab.json', '/home/suryabalaji/GPT_MolBERTa/roberta/tokenizer_folder/merges.txt')
# dataset = Data_Processor('BACE').data_process()
# Data_Split(dataset, 'Class', tokenizer, 512, 4, 0, 0.1, 0.1, 'classification', 'scaffold').get_data_loaders()